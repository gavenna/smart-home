...
void jpeg_effect_display(const char *jpeg_file, int x_start, int y_start, int w, int h)
{
    // printf("%s L_%d\n", __FUNCTION__, __LINE__);
    //  -（1）分配并且初始化一个JPEG对象
    ...
    while (dinfo.output_scanline < dinfo.output_height)
    {
        ...
        for (int x = 0; x < dinfo.output_width; x++)
        {
            ...
            if (x >= x_start && x < x_start + w && dinfo.output_scanline >= y_start && dinfo.output_scanline < y_start + h + 1)
            {
                lcd_draw_point(0 + x, 0 + dinfo.output_scanline - 1, color);
            }
            else if (dinfo.output_scanline > (y_start + h + 1))
            {
                return;
            }
        }
...
//             if ((x == x0 && y > y0 && y < y0 + h - 1) || (x == x0 + w - 1 && y > y0 && y < y0 + h) || y == y0 || y == (y0 + h - 1))
//         }
//     }
